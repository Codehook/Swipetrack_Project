...
if ($_GET['action'] == 'addTopping') {
...
else if ($_GET['action'] == 'removeTopping') {
	$result = array();
	$result['errormsg'] = '';
	$result['success'] = 0;
	...
	if(isset($_GET['topping'])) {
		...
		$_SESSION['toppings'] = array_diff($_SESSION['toppings'],[$_GET['topping']]);
		$_SESSION['toppings'] = array_values($_SESSION['toppings']);
		$result['success'] = 1;
	}
	else {
		$result['errormsg'] = 'No Topping Entered to Remove';
	}

	echo json_encode($result);
	exit;
}
else if ($_GET['action'] == 'removeAllToppings') {
	$result = array();
	$result['errormsg'] = '';
	$result['success'] = 0;
	...
	if(!isset($_SESSION['toppings'])) {
		$_SESSION['toppings'] = array();
		$result['errormsg'] = 'No Toppings to Remove';
	}
	else {
